...
int ServerClass::Send(int client_socket,const char * data,unsigned int bytes,unsigned int flags)
{
    if (g_DebugFile != NULL)
    {
        fwrite(data,1,bytes,g_DebugFile);
    }
    return send(client_socket,data,bytes,flags);
}
